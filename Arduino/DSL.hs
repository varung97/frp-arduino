...
first :: (Stream a -> Stream b) -> (Stream (a, c) -> Stream (b, c))
first fn = \stream ->
  Stream $ do
    sN <- unStream stream
    let s1 = Stream $ do
          s1N <- addAnonymousStream $ DAG.Map $ DAG.TupleValue 0 $ DAG.Input 0
          addDependency sN s1N
    let s2 = Stream $ do
...
second fn = arr swapE >>> first fn >>> arr swapE
  where
    swapE = pack2 . swap . unpack2
    swap (x,y) = (y,x)

(&&&) :: (Stream a -> Stream b) -> (Stream a -> Stream c) -> (Stream a -> Stream (b, c))
...
(***) :: (Stream a -> Stream b) -> (Stream c -> Stream d) -> (Stream (a, c) -> Stream (b, d))
